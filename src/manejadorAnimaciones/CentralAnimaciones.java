...
public class CentralAnimaciones implements ManejadorAnimaciones {
	...
	public void animar_gravedad(Celda celda) {
		Animador animador = new AnimadorGravedad(this, 10, 30, celda);
		agregar_animador_y_lanzar_pendientes(animador);
	}

	public void animar_caida(Celda celda) {
		Animador animador = new AnimadorCaida(this, 10, 30, celda);
		agregar_animador_y_lanzar_pendientes(animador);
	}
	...
	protected void lanzar_pendientes() {
		List<Animador> animadores_a_lanzar = new ArrayList<Animador>();

		synchronized (this) {
			while (existen_pendientes() && existe_proxima_animacion_preparada()) {
				Animador animador = animadores_pendientes.remove(0);
				int prioridad_animador = animador.get_prioridad();
				...
				animadores_a_lanzar.add(animador);
			}
		}

		for (Animador animador : animadores_a_lanzar) {
	...
	protected void quitar_animador_en_progreso(Animador animador) {
		...
		if (animadores_celda != null) {
			...
			if (animadores_celda.isEmpty()) {
				mapeo_celda_animadores.remove(celda_animador);
			}
		}
	}
